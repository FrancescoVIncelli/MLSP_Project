...
def calc_PER(pred, ground_truth, normalize=True, merge_phn=True):
    ...
    pred_seq_list = seq_to_single_char_strings(tensor_to_seq_list(pred, merge_phn=merge_phn))
    truth_seq_list = seq_to_single_char_strings(tensor_to_seq_list(ground_truth, merge_phn=merge_phn))
    ...
    for i in range(len(truth_seq_list)):
        ...
        dist_i = leven.distance(pred_seq_list[i], truth_seq_list[i])
        if normalize:
...
def tensor_to_seq_list(seqs, seq_length=None, merge_phn=True):
    phonemes_list = []
    num_samples = seqs.shape[0] # len(seq)
    
    for n in range(num_samples):
        _seq = [IDX_MAPPING[p] for p in seqs[n]] if merge_phn else seqs[n]
        phonemes_list.append(_seq)
