...
def calc_feat_delta(signal, feat_args, samplerate=16000, normalize=True):
    ...
    # Get parameters for Mel-Ceptra coefficient extraction
    mode        = feat_args.mode
    win_len     = feat_args.win_len
    win_step    = feat_args.win_step
    delta_order = feat_args.delta
    num_cep     = feat_args.num_cep
    num_filt    = feat_args.num_filt  # ( = 2*feat_len )
    feat_len    = feat_args.feat_len
    
    pre_emph    = feat_args.pre_emph
    add_energy  = feat_args.add_energy
    NFFT        = feat_args.nfft
    
    low_freq    = feat_args.low_freq
    high_freq   = feat_args.high_freq
    
    
    feat = calcMFCC(signal, samplerate, win_len, win_step, feat_len, num_filt, NFFT,
                    low_freq, high_freq, pre_emph, num_cep, add_energy, mode=mode)
    
    if normalize:
        feat = ( feat - np.mean(feat, axis=0) ) / np.std(feat, axis=0) # (0.5 * np.std(feat, axis=0))
    
    out = feat
    if delta_order >= 1:
        feat_delta = delta(feat)
        out = np.concatenate((feat, feat_delta), axis=1)
        
        if delta_order >= 2:
            feat_delta_delta = delta(feat_delta)
            out = np.concatenate((feat, feat_delta, feat_delta_delta), axis=1)
            
            if delta_order >= 3:
                feat_delta_delta_delta = delta(feat_delta_delta)
                out = np.concatenate((feat, feat_delta, feat_delta_delta, feat_delta_delta_delta), axis=1)
